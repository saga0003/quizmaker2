...
begin
  ...
  if not public.analytics_can_view_student_v10(v_student) then raise exception 'You do not have access to this student taxonomy evidence.' using errcode='42501'; end if;
  return (
    with selected_papers as (
      ...
      select coalesce(subject.name,paper_section.subject_key,paper_section.title,'General') subject_name,
        question.chapter_id,coalesce(chapter.name,'Unassigned chapter') chapter_name,
        question.topic_id,coalesce(topic.name,'Unassigned topic') topic_name,
        coalesce(response.marks_awarded,0)::numeric marks_awarded,paper_question.marks::numeric maximum_marks,
      ...
      select coalesce(chapter_id,gen_random_uuid()) id,subject_name,chapter_id,chapter_name,count(*)::integer questions,
        sum(correct)::integer correct,sum(incorrect)::integer incorrect,sum(unanswered)::integer unanswered,
        round(sum(marks_awarded),2) marks_awarded,round(sum(maximum_marks),2) maximum_marks,
      ...
      from evidence group by subject_name,chapter_id,chapter_name
    ), topics as (
      select coalesce(topic_id,gen_random_uuid()) id,subject_name,chapter_id,chapter_name,topic_id,topic_name,count(*)::integer questions,
        sum(correct)::integer correct,sum(incorrect)::integer incorrect,sum(unanswered)::integer unanswered,
        round(sum(marks_awarded),2) marks_awarded,round(sum(maximum_marks),2) maximum_marks,
        round(100*sum(marks_awarded)/greatest(sum(maximum_marks),1),1) percentage,
        round(100*sum(correct)::numeric/greatest(count(*),1),1) accuracy,
        round(100*(sum(correct)+sum(incorrect))::numeric/greatest(count(*),1),1) attempt_rate,
        round(avg(time_spent_seconds),1) average_time_seconds
      from evidence group by subject_name,chapter_id,chapter_name,topic_id,topic_name
    )
    select jsonb_build_object(
      'chapters',coalesce((select jsonb_agg(to_jsonb(row) order by row.subject_name,row.chapter_name) from chapters row),'[]'::jsonb),
      'topics',coalesce((select jsonb_agg(to_jsonb(row) order by row.subject_name,row.chapter_name,row.topic_name) from topics row),'[]'::jsonb),
      'generated_at',now()
    )
  );
end;
...
values(null,'analytics.reference_taxonomy_detail_ready','system','39b_v10_reference_taxonomy_detail',
  jsonb_build_object('raw_marks',true,'attempt_rate',true,'average_response_time',true));
