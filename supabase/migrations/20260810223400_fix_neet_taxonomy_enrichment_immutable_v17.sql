...
begin
 ...
 for r in select value from jsonb_array_elements(coalesce(p_rows,'[]'::jsonb)) loop
  ...
  update public.question_staging set
   official_exam='NEET', official_syllabus_code=case when legacy then 'NEET-PYQ-LEGACY' else 'NEET-UG-CURRENT' end, official_unit=case when cid is not null then ch else null end,
   ...
   answer_evidence=coalesce(answer_evidence,'{}'::jsonb)||jsonb_build_object('taxonomy_candidate_chapter',r->>'taxonomy_candidate_chapter','taxonomy_candidate_topic',r->>'taxonomy_candidate_topic','mapped_chapter',ch,'mapped_topic',tp,'chapter_confidence',r->>'taxonomy_chapter_confidence','topic_confidence',r->>'taxonomy_topic_confidence','biology_division',r->>'biology_division','legacy_pyq',legacy),
