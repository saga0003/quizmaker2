...
begin
  ...
  if not public.analytics_can_view_student_v10(v_student) then raise exception 'You do not have access to this student evidence.' using errcode='42501'; end if;
  return (
    with selected_papers as (
      select distinct paper.id from public.question_papers paper
      ...
      select subject_name,chapter_id,chapter_name,topic_id,topic_name,difficulty,count(*)::integer questions,
      ...
      from evidence group by subject_name,chapter_id,chapter_name,topic_id,topic_name,difficulty
    ), type_rows as (
      select subject_name,chapter_id,chapter_name,topic_id,topic_name,question_type,count(*)::integer questions,
        sum(correct)::integer correct,sum(incorrect)::integer incorrect,sum(unanswered)::integer unanswered,
        round(100*sum(correct)::numeric/greatest(count(*),1),1) percentage,
        round(avg(time_spent_seconds),1) average_time_seconds
      from evidence group by subject_name,chapter_id,chapter_name,topic_id,topic_name,question_type
    )
...
values(null,'analytics.reference_scoped_breakdowns_ready','system','39c_v10_reference_breakdown_scope',
  jsonb_build_object('subject',true,'chapter',true,'topic',true,'difficulty',true,'question_types',true));
