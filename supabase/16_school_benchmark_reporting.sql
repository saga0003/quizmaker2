...
begin
 ...
 if v_total>=b.minimum_sample_size and v_school_avg is not null then select round(100.0*count(*)/v_total,1) into v_percentile from public.benchmark_attempt_facts where benchmark_id=b.id and is_valid and score/max_marks*100<=v_school_avg;end if;
...
language sql stable security definer set search_path=public as $$
 with benchmark as(
  ...
  select f.*,round(f.score/f.max_marks*100,2) percentage from public.benchmark_attempt_facts f join benchmark b on b.id=f.benchmark_id where f.is_valid
 ),privacy as(
  select b.minimum_sample_size,count(v.id) valid_attempts from benchmark b left join valid v on true group by b.minimum_sample_size
 )
 select own.student_id,coalesce(p.full_name,p.username,'Student'),trim(concat(m.grade,case when nullif(m.section,'') is null then '' else '-'||m.section end)),own.score,own.max_marks,own.percentage,
  case when privacy.valid_attempts>=privacy.minimum_sample_size then round(100.0*(select count(*) from valid x where x.percentage<=own.percentage)/nullif(privacy.valid_attempts,0),1) else null end,
  own.submitted_at,coalesce(seg.segment_label,'Not enough evidence')
 from valid own join benchmark b on b.id=own.benchmark_id cross join privacy left join public.profiles p on p.id=own.student_id
