...
function DialogContent({
  ...
  const hasCustomMaxWidth = classTokens.some((token) => token.includes("max-w-"))
  const managesInternalScroll = classTokens.includes("overflow-hidden")
  const isWorkspaceDialog = hasCustomMaxWidth && managesInternalScroll
    ...
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          ...
          managesInternalScroll
            ? "flex min-h-0 flex-col gap-0"
            : "grid max-h-[calc(100dvh-2rem)] gap-4 overflow-y-auto",
          !hasCustomMaxWidth && "sm:max-w-lg",
          className,
          isWorkspaceDialog &&
            "h-[calc(100dvh-1rem)] max-h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] sm:h-[92vh] sm:max-h-[920px] sm:w-[calc(100vw-2rem)]"
        )}
        {...props}
      >
        {children}
