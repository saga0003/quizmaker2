...
const demo:PaperListRow[]=[
 ...
 {id:"demo-paper-2",organization_id:null,title:"Physics Unit Test 04",code:"PHY-U04",description:"School unit test",exam_type:"School",status:"draft",duration_minutes:60,total_marks:100,total_questions:40,access_mode:"code",available_from:null,available_until:null,attempt_limit:1,result_mode:"score_and_solutions",created_at:now,updated_at:now},
 {id:"demo-paper-3",organization_id:null,title:"KCET Mathematics Mock",code:"KCET-M02",description:"Entrance practice",exam_type:"KCET",status:"archived",duration_minutes:80,total_marks:60,total_questions:60,access_mode:"private",available_from:null,available_until:null,attempt_limit:2,result_mode:"score_only",created_at:now,updated_at:now},
];
...
export function QuestionPaperList({kind}:{kind:"admin"|"school"}){
  ...
  const columns=useMemo<DataColumn<PaperListRow>[]>(()=>[
    ...
    {key:"status",label:"Status",value:paper=>paper.status,render:paper=><span className={`so-status ${paper.status==="published"?"success":paper.status==="draft"?"neutral":"neutral"}`}>{paper.status}</span>,filter:{label:"statuses",value:paper=>paper.status}},
