...
function isRecheckQuestion(question:QuestionRow){
 return (question.tags||[]).includes("Re-Check")&&["draft","in_review"].includes(question.status);
}
...
export function QuestionBank({kind}:{kind:"admin"|"school"}){
 ...
 const [view,setView]=useState<"all"|"recheck">("all");
 ...
 const filtered=useMemo(()=>questions.filter(q=>{
  if(view==="recheck"&&!isRecheckQuestion(q))return false;
  ...
  return true;
 }),[questions,view,search,status,subject,scope]);
 ...
 const recheckCount=useMemo(()=>questions.filter(isRecheckQuestion).length,[questions]);
 return <div>
  ...
  <section className="rm-card" style={{display:"flex",gap:8,alignItems:"center",padding:7,marginTop:18,width:"fit-content",maxWidth:"100%",overflowX:"auto"}}>
   <button type="button" onClick={()=>setView("all")} style={{border:0,borderRadius:9,padding:"9px 14px",fontWeight:800,whiteSpace:"nowrap",cursor:"pointer",background:view==="all"?"#131e35":"transparent",color:view==="all"?"white":"#667085"}}>All questions</button>
   <button type="button" onClick={()=>{setView("recheck");setStatus("all");setScope("all")}} style={{display:"inline-flex",alignItems:"center",gap:7,border:0,borderRadius:9,padding:"9px 14px",fontWeight:800,whiteSpace:"nowrap",cursor:"pointer",background:view==="recheck"?"#fff6d8":"transparent",color:view==="recheck"?"#775600":"#667085"}}><BookOpenCheck size={16}/> Re-Check <span style={{display:"inline-flex",minWidth:22,height:22,alignItems:"center",justifyContent:"center",borderRadius:999,background:view==="recheck"?"#f6b100":"#eef1f5",color:"#131e35",fontSize:11}}>{recheckCount}</span></button>
  </section>
  {view==="recheck"&&<div style={{marginTop:10,padding:"11px 13px",borderRadius:11,background:"#fffaf0",border:"1px solid #f7d77b",color:"#775600",fontSize:13}}><strong>Teacher review workflow:</strong> edit the draft, verify the question and answer, then change its status to <strong>in review</strong>. A platform admin provides the final approval.</div>}
  ...
  <section className="rm-card" style={{marginTop:16,overflow:"hidden"}}>{loading?<div style={{padding:36,textAlign:"center",color:"#667085"}}><LoaderCircle className="spin" size={22}/> Loading question bank…</div>:filtered.length===0?<div style={{padding:42,textAlign:"center"}}><Filter size={28} color="#98a2b3"/><h3>No matching questions</h3><p style={{color:"#667085"}}>{view==="recheck"?"There are no draft or in-review Re-Check questions for this account.":"Create a question or change the filters."}</p></div>:<div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13,minWidth:920}}><thead><tr style={{textAlign:"left",background:"#f8fafc",color:"#667085"}}><th style={{padding:13}}>Question</th><th>Subject</th><th>Type</th><th>Difficulty</th><th>Ownership</th><th>Status</th><th>Marks</th><th>Version</th><th></th></tr></thead><tbody>{filtered.map(q=>{const style=statusStyle[q.status];const StatusIcon=style.icon;const recheck=(q.tags||[]).includes("Re-Check");return <tr key={q.id} style={{borderTop:"1px solid #eef1f5"}}><td style={{padding:13,maxWidth:380}}><strong style={{display:"block",lineHeight:1.45}}>{q.stem_text}</strong><div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>{recheck&&<span className="rm-badge" style={{padding:"3px 7px",background:"#fff6d8",color:"#775600",border:"1px solid #f7d77b"}}>Re-Check</span>}{(q.exam_types||[]).slice(0,3).map(x=><span className="rm-badge" key={x} style={{padding:"3px 7px",background:"#f2f4f7"}}>{x}</span>)}</div></td><td>{q.subjects?.name||"Unclassified"}<div style={{fontSize:11,color:"#98a2b3",marginTop:3}}>{q.chapters?.name||"No chapter"}</div></td><td>{q.question_type.replaceAll("_"," ")}</td><td style={{textTransform:"capitalize"}}>{q.difficulty.replaceAll("_"," ")}</td><td><span className="rm-badge" style={{background:q.organization_id?"#f4ebff":"#fff6d8",color:q.organization_id?"#6941c6":"#775600"}}>{q.organization_id?"School":"ScholarOS"}</span></td><td><span className="rm-badge" style={{background:style.bg,color:style.color}}><StatusIcon size={13}/>{q.status.replaceAll("_"," ")}</span></td><td>{q.marks} / −{q.negative_marks}</td><td>v{q.version_number}</td><td>{!q.id.startsWith("demo")&&<Link href={`${base}/new/?id=${q.id}`} style={{display:"inline-flex",alignItems:"center",gap:5,fontWeight:750,color:"#775600"}}><Pencil size={15}/> Edit</Link>}</td></tr>})}</tbody></table></div>}</section>
