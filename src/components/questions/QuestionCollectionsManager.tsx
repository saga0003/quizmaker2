  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <p className="mt-1 max-w-3xl text-sm leading-6 text-[var(--muted-foreground)]">Create named, ordered groups from approved questions and reuse them across papers, products and practice workflows without duplicating the question records.</p>
      </div>
    ...
    {loading ? <div className="grid min-h-[300px] place-items-center text-sm text-[var(--muted-foreground)]"><div><LoaderCircle className="mx-auto mb-3 h-7 w-7 animate-spin" />Loading Question Collections…</div></div> : !filteredCollections.length ? <Card className="border-dashed border-[#C6D1D9] shadow-none"><CardContent className="grid min-h-[280px] place-items-center text-center"><div><FolderOpen className="mx-auto mb-4 h-10 w-10 text-[#89AAAA]" /><h2 className="font-semibold text-[#071D34]">No Question Collections yet</h2><p className="mt-2 text-sm text-[var(--muted-foreground)]">Create the first reusable question group from the approved Question Bank.</p><Button className="mt-5 bg-[#006B70] text-white" onClick={openCreate}>Create collection</Button></div></CardContent></Card> : <div className="grid gap-4 xl:grid-cols-2">
      {filteredCollections.map((collection) => <Card key={collection.id} className="border-[#DFE6EC] shadow-[0_10px_30px_rgba(5,31,50,.045)]"><CardContent className="p-5">
        ...
        <div className="mt-5 flex flex-wrap gap-2 border-t border-[#E8EDF1] pt-4">
          ...
          {collection.can_manage && <Button size="sm" variant="ghost" className="text-[var(--destructive)]" onClick={() => setDeleteTarget(collection)}><Trash2 className="mr-2 h-3.5 w-3.5" />Delete</Button>}
        </div>
      </CardContent></Card>)}
    </div>}
    ...
    <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}><DialogContent><DialogHeader><DialogTitle>Delete Question Collection?</DialogTitle><DialogDescription>This deletes only the collection and its links. The original questions and any paper already created from it remain untouched.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setDeleteTarget(null)}>Cancel</Button><Button onClick={() => void deleteCollection()} className="bg-[var(--destructive)] text-white hover:bg-[#963B3B]" disabled={saving}>{saving && <LoaderCircle className="mr-2 h-4 w-4 animate-spin" />}Delete collection</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}
