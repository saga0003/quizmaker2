...
export function AppSidebar() {
  ...
  return (
    <aside className={`fixed left-0 top-0 z-40 flex h-screen flex-col bg-[#14232B] text-white transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-[68px]'}`}>
      ...
      <div className="p-3">
        {sidebarOpen ? (
          ...
          <Tooltip delayDuration={0}><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={() => void logout()} className="mx-auto flex text-white/40 hover:bg-white/10 hover:text-[#B54747]"><LogOut className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent side="right">Sign Out</TooltipContent></Tooltip>
        )}
      </div>
    </aside>
  );
