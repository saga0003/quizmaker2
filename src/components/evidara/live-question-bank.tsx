...
export function LiveQuestionBank({ kind }: { kind: 'admin' | 'school' }) {
  ...
  useEffect(() => {
    setPage(1);
  }, [bank, selectedSchoolId, search, subjectFilter, chapterFilter, topicFilter, statusFilter, difficultyFilter, gradeFilter, examFilter, dateMode, dateFrom, dateTo, sort]);
  ...
  async function confirmDelete() {
    if (!supabase || !canDelete || !deleteIds.length) return;
    setDeleting(true);
    setError('');
    setMessage('');
    const { data, error: deleteError } = await supabase.rpc('bulk_delete_questions_v71', {
      p_question_ids: deleteIds,
    });
    if (deleteError) {
      const migrationMissing = /bulk_delete_questions_v71|does not exist|not found/i.test(deleteError.message);
      setError(migrationMissing
        ? 'Bulk deletion is not ready. Apply Supabase migration 31 and refresh the page.'
        : deleteError.message);
      setDeleting(false);
      return;
    }

    const summary = (data || {}) as DeleteSummary;
    const deleted = Array.isArray(summary.deleted) ? summary.deleted : [];
    ...
    if (failed.length) {
      setError(`${deleted.length} question${deleted.length === 1 ? '' : 's'} deleted. ${failed.length} could not be deleted: ${failed.slice(0, 3).map((item) => item.error || 'Linked question').join(' ')}`);
    } else {
      setMessage(`${deleted.length} question${deleted.length === 1 ? '' : 's'} permanently deleted and recorded in the deletion audit.`);
    }
    await load();
    setDeleting(false);
  }
  ...
  const deletePreview = deleteIds
    .map((id) => questions.find((question) => question.id === id)?.stem_text)
    .filter(Boolean)
    .slice(0, 4) as string[];
          ...
          {canDelete && (
            <div className="flex flex-col gap-3 rounded-xl border border-[#DCE9E7] bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button type="button" variant="destructive" onClick={() => requestDelete([...selectedIds])} disabled={!selectedIds.size || deleting}>
                {deleting ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <Trash2 className="mr-2 h-4 w-4" />}Delete selected
              </Button>
            </div>
          )}
