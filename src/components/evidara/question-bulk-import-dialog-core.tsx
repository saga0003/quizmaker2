  ...
  async function parse(file: File) {
    ...
    try {
      if (extension === 'zip') {
        ...
        const candidates = entries.filter((entry) => /\.(tex|csv|xlsx|xls|json|docx|pdf|txt)$/i.test(entry.name));
        ...
        setImageZip(file);
        setZipNames(names);
        setNotice(`ZIP ready. Evidara automatically chose ${sourceName} as the question paper${ignoredSupportFiles.length ? ` and ignored ${ignoredSupportFiles.length} supporting report file${ignoredSupportFiles.length === 1 ? '' : 's'}` : ''}. Bundled images will be matched automatically.`);
      } else {
        setImageZip(null);
        setZipNames(null);
      }
        ...
        <DialogContent className="max-h-[96vh] w-[96vw] max-w-[1540px] overflow-hidden border-[#E7ECEB] p-0">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto bg-[#FBFCFC] px-3 py-4 sm:px-6 sm:py-5">
            <div className="mb-5 grid gap-3 rounded-2xl border border-[#E7ECEB] bg-white p-4 sm:grid-cols-4">
              {[['1','Upload','Choose source files or content'],['2','Map','Confirm academic mapping'],['3','Review','Fix only what needs attention'],['4','Import','Save ready questions']].map(([step,title,sub], index) => <div key={step} className="flex items-start gap-3"><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full text-sm font-bold ${index === 0 ? 'bg-[#0E5A5A] text-white' : 'border border-[#D9E1E4] bg-[#F7F9F9] text-[#596A70]'}`}>{step}</span><span><strong className="block text-xs text-[#14232B]">{title}</strong><span className="mt-0.5 block text-[11px] leading-4 text-[#7B8A90]">{sub}</span></span></div>)}
            </div>
