...
export function SubscriptionCenter() {
  ...
  const licensed = Math.max(0, Number(subscription.seatLimit || 0));
  const available = Math.max(0, licensed - used);
  const overLicence = Math.max(0, used - licensed);
  const placeholderSubscription = !active && licensed <= 0 && subscription.startsAt === subscription.endsAt;
  const planName = /scholar|evidara\s+annual\s+school\s+access/i.test(subscription.planName) ? "Evidara Institution Plan" : subscription.planName;
  ...
  const licenceValue = licensed > 0 ? licensed.toLocaleString("en-IN") : "Not licensed";
  const licenceNote = licensed <= 0
    ? "Set the licensed student quantity to activate access"
    : overLicence > 0
      ? `${overLicence.toLocaleString("en-IN")} active student${overLicence === 1 ? " is" : "s are"} over the current licence`
      : `${available.toLocaleString("en-IN")} licence${available === 1 ? "" : "s"} available`;
  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div>
        ...
        <p className="mt-1 text-sm text-[#6B7980]">Access, resources and annual student licensing for {state.school.name}.</p>
      </div>
    ...
    {overLicence > 0 && <div className="rounded-xl border border-[#F1D8A5] bg-[#FFF9EC] px-4 py-3 text-sm text-[#7A5A10]">This institution currently has {used.toLocaleString("en-IN")} active students against {licensed.toLocaleString("en-IN")} licensed students. Increase the licensed quantity before adding or activating more students.</div>}
    ...
    <Card className="overflow-hidden border-[#DCE6E9] shadow-sm">
      <CardContent className="p-0">
        <div className="grid gap-0 lg:grid-cols-[1.4fr_1fr_1fr]">
          ...
          <div className="border-t border-[#EDF1F2] p-6 lg:border-l lg:border-t-0">
            <div className="grid grid-cols-2 gap-5"><div><Users className="h-5 w-5 text-[#31566B]" /><p className="mt-2 text-xs text-[#7B8A90]">Licensed students</p><strong className="text-[#14232B]">{licenceValue}</strong><p className="mt-1 text-xs text-[#7B8A90]">{used.toLocaleString("en-IN")} active now</p></div><div><FileText className="h-5 w-5 text-[#31566B]" /><p className="mt-2 text-xs text-[#7B8A90]">Tests</p><strong className="text-[#14232B]">Unlimited</strong><p className="mt-1 text-xs text-[#7B8A90]">No per-test charge</p></div><div className="col-span-2"><ShieldCheck className="h-5 w-5 text-[#31566B]" /><p className="mt-2 text-xs text-[#7B8A90]">Resource access</p><strong className="text-[#14232B]">{subscription.resourceAccess === "full" ? "Included" : "Limited"}</strong></div></div>
          </div>
        </div>
      </CardContent>
    </Card>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      {[
        ...
        [ShieldCheck, "Licensed students", licenceValue, licenceNote],
      ...
      ].map(([Icon, label, value, note]) => <Card key={String(label)} className="border-[#DFE6EC] shadow-sm"><CardContent className="p-4"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#EAF6F4] text-[#0E5A5A]"><Icon className="h-4 w-4" /></div><p className="mt-3 text-xs text-[#7B8A90]">{label as string}</p><p className="mt-1 break-words text-lg font-bold text-[#14232B]">{value as string}</p><p className="mt-1 text-xs leading-5 text-[#7B8A90]">{note as string}</p></CardContent></Card>)}
    </div>
