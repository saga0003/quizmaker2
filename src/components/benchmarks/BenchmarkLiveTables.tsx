...
export function BenchmarkLiveTables({snapshot,students,organizationName}:{snapshot:Snapshot|null;students:StudentRow[];organizationName:string}){
 const studentColumns=useMemo<DataColumn<StudentRow>[]>(()=>[
  ...
  {key:"percentile",label:<MetricLabel {...metricDefinitions.percentile}>Percentile</MetricLabel>,value:r=>Number(r.percentile),render:r=>`${Math.round(Number(r.percentile))}th`,align:"right"},
 ...
 return <><div className="so-grid so-grid-2 so-mt"><section className="so-card so-pad"><div className="so-section-head"><div><span className="so-kicker">GLOBAL DISTRIBUTION</span><h2>{snapshot?.available?"Where participants scored":"Waiting for the privacy minimum"}</h2></div><MetricInfo {...metricDefinitions.benchmark}/></div>{snapshot?.available?<SortableDataTable rows={dist} columns={distColumns} rowKey={r=>r.band}/>:<div className="ev-empty-benchmark"><ShieldCheck/><p>Wider statistics remain hidden until {snapshot?.minimum_sample_size||20} verified attempts.</p></div>}</section><section className="so-card so-pad"><span className="so-kicker">RESPONSIBLE USE</span><h2>What the benchmark can say</h2><div className="so-steps"><div><b>1</b><span><strong>Exact-version evidence</strong><small>Only the same published paper is included.</small></span></div><div><b>2</b><span><strong>Trusted result facts</strong><small>The server derives marks from the completed attempt.</small></span></div><div><b>3</b><span><strong>Context, not ranking</strong><small>No public school name or external student row is shown.</small></span></div></div></section></div><section className="so-card so-pad so-mt"><div className="so-section-head"><div><span className="so-kicker">YOUR SCHOOL · PRIVATE VIEW</span><h2>Students on this shared paper</h2></div><span className="so-status neutral">{students.length} attempts</span></div><SortableDataTable rows={students} columns={studentColumns} rowKey={r=>`${r.student_id}-${r.submitted_at}`} searchText={r=>`${r.student_name} ${r.grade} ${r.segment_label} ${r.score}`} searchPlaceholder="Search student, grade, marks or segment" initialSortKey="score" initialSortDirection="desc"/></section></>;
}
