...
export function DashboardShell({ kind, children }: { kind: Kind; children: React.ReactNode }) {
  ...
  return <div className="so-shell">
    <aside className="so-sidebar">
      <div className="so-sidebar-brand"><Link href="/"><Logo/></Link><button className="so-collapse"><ChevronRight size={15}/></button></div>
      ...
      <div className="so-sidebar-card"><Sparkles size={18}/><div><strong>ScholarOS Annual</strong><span>{kind === "student" ? "Resources assigned by your school" : "Tests free · PYQs complimentary"}</span></div></div>
      <div className="so-account">
        <div className="so-avatar">{avatarText}</div>
        <div><strong>{displayName}</strong><span>{displayRole.replaceAll("_", " ")}{demoIdentity?.id ? ` · ${demoIdentity.id}` : ""}</span></div>
        {(user || demoIdentity) && <button onClick={() => void logout()} title="Sign out"><LogOut size={17}/></button>}
      </div>
    </aside>
    <main className="so-main">
      <header className="so-topbar"><div><span className="so-live-dot"/> {configured ? "Cloud data connected" : "Interactive demo mode"}</div><div className="so-top-summary"><span><CalendarRange size={15}/> Academic year 2026–27</span><span><Layers3 size={15}/> ScholarOS V5</span></div></header>
