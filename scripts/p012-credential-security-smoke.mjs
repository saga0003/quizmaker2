...
const checks = [
  ...
  ['security gate refreshes an expired session once', /auth\.refreshSession\(\)/.test(gate) && /requestSecurity\(refreshedToken\)/.test(gate)],
  ['security gate has a recoverable failed-session screen', /verify this session/.test(gate) && /Sign in again/.test(gate) && /Retry/.test(gate)],
  ['stale auth callback errors are removed from the URL', /error_code/.test(gate) && /window\.history\.replaceState/.test(gate)],
