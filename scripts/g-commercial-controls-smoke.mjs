...
const onboardingMigration = read('supabase/migrations/20260913030000_institution_onboarding_v2.sql');
...
check('G1 canonical plan is ₹199 times licensed students for annual period', /₹199 per licensed student for the annual licence period/i.test(subscriptionCenter) && /RATE_PAISE\s*=\s*19900/.test(adminControl) && /annual_price_per_student_paise\s*=\s*19900/i.test(canonicalPlan));
...
check('G3 Super Admin cannot override canonical annual rate', /annual_price_per_student_paise:\s*19900/.test(schoolRoute) && /annual_price_per_student_paise[\s\S]*19900/.test(onboardingMigration) && !/annual_price_per_student_paise:\s*(?:Number|String)\(/.test(adminControl));
check('G3 Super Admin displays annual amount from licensed quantity', /const annual\s*=\s*Math\.max\(0,\s*Number\(sub\.seat_limit\s*\|\|\s*0\)\)\s*\*\s*RATE_PAISE/.test(adminControl) && /Estimated annual licence|Annual licence amount/.test(adminControl));
...
check('G6 manual payment record is supported in rupees and stored server-side in paise', /Payment record/i.test(adminControl) && /Amount received \(₹\)/i.test(adminControl) && /manual_amount_paise/.test(schoolRoute) && /payment_status/.test(schoolRoute));
check('G6 activation remains Super-Admin controlled without checkout dependency', /Institution controlled by Evidara/i.test(subscriptionCenter) && /action === 'activate'/.test(schoolRoute) && /Super Admin permission is required/.test(schoolRoute) && !/checkout|payment gateway/i.test(onboardingRoute));
