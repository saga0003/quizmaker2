...
check('G1 canonical plan is ₹199 times licensed students for annual period', /₹199 per licensed student for the annual licence period/i.test(subscriptionCenter) && /ANNUAL_RATE_PAISE\s*=\s*19900/.test(adminControl));
...
check('G3 Super Admin displays annual amount from licensed quantity', /Annual licence amount/.test(adminControl) && /Number\(subForm\.seat_limit\s*\|\|\s*0\)\s*\*\s*ANNUAL_RATE_PAISE/.test(adminControl));
