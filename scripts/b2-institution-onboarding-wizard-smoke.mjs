...
const checks = [
  ...
  ['review communicates invitation delivery without backend detail', /Please confirm the details above[\s\S]*School Admin invitation[\s\S]*secure invitation link/],
  ...
  ['server onboarding attempts automatic admin invite', /inviteUserByEmail\(adminEmail/],
  ['server onboarding falls back to secure invitation link', /generateLink\(\{ type: 'invite', email: adminEmail \}\)/],
  ['UI exposes a copyable invitation link when email delivery fails', /Copy invitation link/],
